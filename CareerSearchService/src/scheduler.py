from CareerSearch.gen.ttypes import Status, LinkType
...
def prepare_link_item(item, curosr, when_insert):
    ...
    priority = _assign_priority(item.status, when_insert)
    ...
    if item.pages:
        ...
        if num:
            ...
            priority = _assign_priority(item.status, when_insert, bonus)
...
def _assign_priority(status, when_insert, bonus=None):
...
def _get_pages(url, cursor, pages):
    sql = 'SELECT pages FROM links WHERE link = %s'
